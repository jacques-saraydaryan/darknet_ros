...
detection *YoloObjectDetector::avgPredictions(network *net, int *nboxes)
{
  ...
  detection *dets = get_network_boxes(net, getBuff_(0).w, getBuff_(0).h, demoThresh_, demoHier_, 0, 1, nboxes);
  return dets;
}
...
void *YoloObjectDetector::detectInThread()
{
  ...
  //float *X = buffLetter_[(getBuffIndex_() + 2) % 3].data;
  float *X =getBuffLetter_((getBuffIndex_() + 2) % 3).data;
  ...
  image display = getBuff_((getBuffIndex_()+2) % 3);
...
{
  {
    ...
    //FIXME FATAL HERE may be due to the image size change...
    if (ROS_img->height*ROS_img->width != getBuff_(getBuffIndex_()).h*getBuff_(getBuffIndex_()).w){
      ROS_DEBUG("[YoloObjectDetector-JSA] different size!!!");
      //buff_[buffIndex_]=ipl_to_image(ROS_img);
      //Reset Buff
      image imTmp = ipl_to_image(ROS_img);
      img_to_free_vect.push_back(getBuff_(0));
      //replaceImageBuff(0,imTmp);
      setBuff_(0,imTmp);
      image imTmp2 =copy_image(getBuff_(0));
      //replaceImageBuff(1,imTmp2);
      img_to_free_vect.push_back(getBuff_(1));
      setBuff_(1, imTmp2);
      
      image imTmp3 =copy_image(getBuff_(0));
      //replaceImageBuff(2,imTmp3);
      img_to_free_vect.push_back(getBuff_(2));
      setBuff_(2, imTmp3);

      //CAUTION free is also needed for HeaderBuff and Letterbix ???
      ...
      img_to_free_vect.push_back(getBuffLetter_(0));
      setBuffLetter_(0,letterbox_image(getBuff_(0), net_->w, net_->h));
      img_to_free_vect.push_back(getBuffLetter_(1));
      setBuffLetter_(1,letterbox_image(getBuff_(0), net_->w, net_->h));
      img_to_free_vect.push_back(getBuffLetter_(2));
      setBuffLetter_(2,letterbox_image(getBuff_(0), net_->w, net_->h));
      
      //CAUTION could be source of conflit concurrency access, need to add a lock ??? 
      //ipl_ = cvCreateImage(cvSize(buff_[buffIndex_].w, buff_[buffIndex_].h), IPL_DEPTH_8U, buff_[buffIndex_].c);
      setIpl_(cvCreateImage(cvSize(getBuff_(getBuffIndex_()).w, getBuff_(getBuffIndex_()).h), IPL_DEPTH_8U, getBuff_(getBuffIndex_()).c));

      //CAUTION could be source of conflit concurrency access, need to add a lock ???
      //buffIndex_ = (getBuffIndex_() + 1) % 3;
      ...
      setBuffIndex_(0);
        ...
        ipl_into_image(ROS_img, getBuff_(getBuffIndex_()));
  ...
  rgbgr_image(getBuff_(getBuffIndex_()));
  //letterbox_image_into(getBuff_([getBuffIndex_()]), net_->w, net_->h, buffLetter_[getBuffIndex_()]);
  letterbox_image_into(getBuff_(getBuffIndex_()), net_->w, net_->h, getBuffLetter_(getBuffIndex_()));
...
{
  //if (ipl_->height*ipl_->width == buff_[(buffIndex_ + 1)%3].h*buff_[(buffIndex_ + 1)%3].w){
      //if size differs do not display
    //show_image_cv(buff_[(buffIndex_ + 1)%3], "YOLO V3", ipl_);
    show_image_cv(getBuff_((getBuffIndex_() + 1)%3), "YOLO V3", getIpl_());
  //}
...
void YoloObjectDetector::yolo()
{
...

  {
    ...
    setBuff_(0,ipl_to_image(ROS_img));
  ...
  setIpl_(cvCreateImage(cvSize(getBuff_(0).w, getBuff_(0).h), IPL_DEPTH_8U, getBuff_(0).c));
  ...
  while (!demoDone_) {
    ...
    if (!demoPrefix_) {
      ...
      if (viewImage_) {
        ...
        generate_image(getBuff_((getBuffIndex_() + 1)%3), getIpl_());
      ...
      save_image(getBuff_((getBuffIndex_() + 1) % 3), name);
    }
    fetch_thread.join();
    detect_thread.join();

    
    for(const auto& img: img_to_free_vect) {
         free(img.data); 
    }
    img_to_free_vect.clear();
...
void *YoloObjectDetector::publishInThread()
{
  ...
  IplImage *YoloObjectDetector::getIpl_(){
    {
      boost::unique_lock<boost::shared_mutex> lockImageStatus(mutexIpl_);
      return ipl_;
    }

  }

  void YoloObjectDetector::setIpl_(IplImage *ipl){
    {
      boost::unique_lock<boost::shared_mutex> lockImageStatus(mutexIpl_);
      ipl_ = ipl;
    }
    ...
    int YoloObjectDetector::getBuffIndex_(){
      {
        boost::unique_lock<boost::shared_mutex> lockBuff(mutexBuffIndex_);
        return buffIndex_;
      }

    }

  void YoloObjectDetector::setBuffIndex_(int index){
     {
        boost::unique_lock<boost::shared_mutex> lockBuff(mutexBuffIndex_);
        buffIndex_ = index;
     }
   ...
   image YoloObjectDetector::getBuff_(int index){
     {
  ...
  void YoloObjectDetector::setBuff_(int index,image img){
    {
      boost::unique_lock<boost::shared_mutex> lockBuff(mutexBuff_);
            buff_[index] =img;
     }
  ...
  std_msgs::Header YoloObjectDetector::getHeaderBuff(int index){
{
      boost::unique_lock<boost::shared_mutex> lockBuff(mutexHeaderBuff_);
      return headerBuff_[index];

     }
  }
  void YoloObjectDetector::setHeaderBuff(int index, std_msgs::Header head){
    {
      boost::unique_lock<boost::shared_mutex> lockBuff(mutexHeaderBuff_);
            headerBuff_[index]=head;
     }

  }
  
  image YoloObjectDetector::getBuffLetter_(int index){
    {
      boost::unique_lock<boost::shared_mutex> lockBuff(mutexBuffLetter_);
      return buffLetter_[index];
